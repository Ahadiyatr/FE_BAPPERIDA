...
const LABEL_AKSI: Record<AksiLog, string> = {
  ...
  KUNCI_PERIODE: 'Kunci periode',
  UBAH_BOBOT: 'Ubah bobot',
  TANDAI_SIAP: 'Tandai siap',
  BATAL_SIAP: 'Batal siap',
  SALIN_RENCANA: 'Salin rencana',
  CATAT_REALISASI: 'Catat realisasi',
...
export default function LogAktivitasLayar() {
  ...
  const gaya =
    'rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm text-slate-700 focus:border-emerald-500 focus:outline-none';
    ...
    <div className="space-y-6">
      {peran === 'admin_bidang' && (
        <div className="flex gap-2.5 rounded-xl border border-emerald-100 bg-emerald-50 p-4 text-sm text-emerald-800">
          ...
          <span>
            Anda melihat jejak bidang Anda sendiri. Kejadian bidang lain dan
            perubahan data master tidak ditampilkan.
          </span>
        </div>
      )}
      ...
      <div className="flex flex-wrap items-center gap-2">
        ...
        <select
          className={gaya}
          value={aksi}
          onChange={e => setAksi(e.target.value as AksiLog | '')}
        >
        ...
        <select
          ...
          value={periodeId}
          onChange={e =>
            setPeriodeId(e.target.value === '' ? '' : Number(e.target.value))
          }
        >
                ...
                <td colSpan={5} className="px-6 py-10 text-center">
                  ...
                  {!(cari || aksi || periodeId !== '') && (
                    <p className="mt-1 text-xs text-slate-400">
                      Jejak mulai terisi begitu ada yang mengubah master,
                      mengunci periode, atau mencatat realisasi.
                    </p>
                  )}
                </td>
              </tr>
              ...
              baris.map(l => (
                <tr key={l.id} className="hover:bg-slate-50">
                  ...
                  <td className="px-6 py-3">
                    <span
                      className={`rounded-lg px-2 py-0.5 text-xs font-semibold whitespace-nowrap ${NADA_AKSI[l.aksi]}`}
                    >
                      {LABEL_AKSI[l.aksi]}
                    </span>
                  </td>
