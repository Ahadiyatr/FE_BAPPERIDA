...
export default function MasterIndikator() {
  ...
  const [indikatorUtamaList, setIndikatorUtamaList] = useState<IndikatorUtama[]>([]);
  ...
  const handleCloseModal = () => {
    ...
    setFormData({ INDIKATOR_UTAMA_ID: "", KODE_INDIKATOR: "", NAMA_INDIKATOR: "", SATUAN: "", FLAG_ACTIVE: true });
    setCurrentId(null);
  };
  ...
  const executeDelete = async () => {
    ...
    setIsDeleting(true);
    try {
      await api.delete(`/api/master-indikator/${deleteId}`);
      fetchData(); // Refresh data
      cancelDelete();
    } catch (err: any) {
  ...
  const filteredData = data.filter((item) =>
    (item.NAMA_INDIKATOR?.toLowerCase() || "").includes(search.toLowerCase()) ||
    (item.KODE_INDIKATOR?.toLowerCase() || "").includes(search.toLowerCase()) ||
    (item.indikator_utama?.NAMA_INDIKATOR_UTAMA?.toLowerCase() || "").includes(search.toLowerCase())
  );
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h1 className="text-2xl font-bold text-slate-900">Master Indikator</h1>
          <p className="text-sm text-slate-500 mt-1">Kelola data indikator detail</p>
        </div>
        <button 
                  ...
                  <tr key={item.ID} className="hover:bg-slate-50 transition-colors">
                    ...
                    <td className="px-6 py-4 text-sm text-slate-600">
                      {item.indikator_utama ? (
                        <div>
                          <span className="font-medium text-slate-700">{item.indikator_utama.KODE_INDIKATOR_UTAMA}</span>
                          <span className="mx-1">-</span>
                          {item.indikator_utama.NAMA_INDIKATOR_UTAMA}
                        </div>
                      ) : (
                        <span className="text-slate-400 italic">Tidak ada indikator utama</span>
                      )}
                    </td>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/50 backdrop-blur-sm">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-md overflow-hidden flex flex-col max-h-[90vh]">
            ...
            <div className="overflow-y-auto p-4">
              <form id="indikator-form" onSubmit={handleSubmit} className="space-y-4">
                <div>
                  <label htmlFor="INDIKATOR_UTAMA_ID" className="block text-sm font-medium text-slate-700 mb-1">
                    Indikator Utama Terkait <span className="text-red-500">*</span>
                  </label>
                  <select
                    id="INDIKATOR_UTAMA_ID"
                    required
                    value={formData.INDIKATOR_UTAMA_ID}
                    onChange={(e) => setFormData({ ...formData, INDIKATOR_UTAMA_ID: e.target.value })}
                  ...
                  >
                    <option value="" disabled>Pilih Indikator Utama</option>
                ...
                <div>
                  <label htmlFor="KODE_INDIKATOR" className="block text-sm font-medium text-slate-700 mb-1">
                  ...
                  <input
                    ...
                    onChange={(e) => setFormData({ ...formData, KODE_INDIKATOR: e.target.value })}
                ...
                <div>
                  <label htmlFor="NAMA_INDIKATOR" className="block text-sm font-medium text-slate-700 mb-1">
                ...
                <div>
                  <label htmlFor="SATUAN" className="block text-sm font-medium text-slate-700 mb-1">
                    Satuan
                  </label>
                  <input
                    type="text"
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/50 backdrop-blur-sm">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-sm overflow-hidden">
            <div className="p-6 text-center">
              ...
              <h2 className="text-xl font-bold text-slate-900 mb-2">Hapus Indikator?</h2>
              <p className="text-sm text-slate-500 mb-6">
                Apakah Anda yakin ingin menghapus indikator ini? Tindakan ini tidak dapat dibatalkan.
              </p>
