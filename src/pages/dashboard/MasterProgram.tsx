...
export default function MasterProgram() {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      // Sesuaikan dengan struktur response Laravel (biasanya di response.data.data atau response.data)
  ...
  const handleOpenModal = (mode: "add" | "edit", program?: Program) => {
    setModalMode(mode);
    if (mode === "edit" && program) {
      ...
      setFormData({
        ...
        FLAG_ACTIVE: program.FLAG_ACTIVE === undefined ? true : Boolean(program.FLAG_ACTIVE),
      });
    } else {
      setCurrentId(null);
      setFormData({
  ...
  const handleCloseModal = () => {
    ...
    setFormData({ KODE_PROGRAM: "", NAMA_PROGRAM: "", FLAG_ACTIVE: true });
    setCurrentId(null);
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (modalMode === "add") {
        await api.post("/api/master-program", formData);
      } else if (modalMode === "edit" && currentId) {
        await api.put(`/api/master-program/${currentId}`, formData);
      }
      handleCloseModal();
  ...
  const executeDelete = async () => {
    ...
    setIsDeleting(true);
    try {
      await api.delete(`/api/master-program/${deleteId}`);
      fetchData(); // Refresh data
      cancelDelete();
    } catch (err: any) {
    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          ...
          <p className="text-sm text-slate-500 mt-1">Kelola data master program Bapperrida</p>
        </div>
        <button 
          ...
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-md overflow-hidden">
            <div className="flex items-center justify-between p-4 border-b border-slate-100">
              <h2 className="text-lg font-bold text-slate-900">
                {modalMode === "add" ? "Tambah Program" : "Edit Program"}
              </h2>
              <button 
            ...
            <form onSubmit={handleSubmit} className="p-4 space-y-4">
              <div>
                <label htmlFor="KODE_PROGRAM" className="block text-sm font-medium text-slate-700 mb-1">
              ...
              <div>
                <label htmlFor="NAMA_PROGRAM" className="block text-sm font-medium text-slate-700 mb-1">
                  Nama Program
                </label>
                <textarea
                  id="NAMA_PROGRAM"
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/50 backdrop-blur-sm">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-sm overflow-hidden">
            <div className="p-6 text-center">
              ...
              <h2 className="text-xl font-bold text-slate-900 mb-2">Hapus Program?</h2>
