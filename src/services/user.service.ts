...
export async function simpanUser(i:SimpanUserInput):Promise<User>{const body={name:i.name,email:i.email,role:i.role,bidang_id:i.role==="admin_bidang"?i.bidangId:null};return map(dataOf<Row>(i.id?await api.put(`/pengguna/${i.id}`,body):await api.post("/pengguna",{...body,password:"password"})))}
export async function setAktifUser(id:number,aktif:boolean):Promise<User>{if(!aktif){await api.delete(`/pengguna/${id}`);const r=await getUserById(id);if(!r)throw new Error("Pengguna tidak ditemukan.");return {...r,flagActive:false}}throw new Error("Backend belum menyediakan endpoint pemulihan pengguna.")}
