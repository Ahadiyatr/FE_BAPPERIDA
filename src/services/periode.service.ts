...
type KesiapanRow={nama_bidang:string;status:"DRAFT"|"SIAP"}
...
export async function cekSyaratBuka(id:number,daftar?:Periode[]):Promise<SyaratBukaPeriode>{const [periode,response]=await Promise.all([daftar??getPeriode(),api.get(`/periode/${id}/kesiapan`)]);const papan=dataOf<KesiapanRow[]>(response);const lain=periode.find(p=>p.status==="OPEN"&&p.id!==id);const bidangBelumSiap=papan.filter(b=>b.status!=="SIAP").map(b=>b.nama_bidang);return{boleh:!lain&&papan.length>0&&bidangBelumSiap.length===0,bidangBelumSiap,adaPeriodeLainTerbuka:lain?.namaPeriode??null}}
