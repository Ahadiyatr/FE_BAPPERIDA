...
type RencanaRow={id:number;kode_subkegiatan:string;nama_subkegiatan:string;capaian:number;aktifitas:{id:number;nama_aktifitas:string;tipe_aktifitas:"UTAMA"|"PENDUKUNG";bobot_target:number;target:number;realisasi:number;bobot_realisasi:number}[]}
...
export async function getAktifitasBidang(_bidangId:number,periodeId:number):Promise<AktifitasPencatatan[]>{
  ...
  return rows.flatMap(r=>r.aktifitas.map(a=>({
    ...
    bobotRealisasi:Number(a.bobot_realisasi),jumlahCatatan:0,jumlahLampiran:0,
