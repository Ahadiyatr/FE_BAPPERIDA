...
interface Baris {
  label: string
  nilai: string
  /** Tandai baris hasil akhir — dicetak brass dan tebal. */
  total?: boolean
}
...
export function BobotLedger({
  ...
  return (
    <div
      className={cn(
        "rounded-sm bg-ledger px-4 py-3.5 font-mono text-[11.5px] leading-[1.9] text-ledger-foreground/85",
        className
      )}
      {...props}
    >
      {baris.map((b, i) => (
        <React.Fragment key={i}>
          ...
          <div className="flex gap-2.5">
            <span className={cn("flex-1", b.total && "font-medium text-brass")}>
              {b.label}
            </span>
            <span
              className={cn(
                "tabular font-medium",
                b.total ? "text-brass" : "text-white"
              )}
            >
              {b.nilai}
            </span>
      ...
      {meter && (
        <>
          <div className="my-3 flex h-3.5 gap-[2px]">
            <i className="flex-[70] rounded-[1px] bg-brass" />
            {Array.from({ length: meter.jumlahPendukung }, (_, i) => (
              <i
                key={i}
                className={cn(
                  "flex-[10] rounded-[1px] bg-teal",
                  meter.segmenBaru &&
      ...
      {peringatan && (
        <>
          ...
          <div className="text-amber-tint">{peringatan}</div>
        </>
      )}
    </div>
  )
